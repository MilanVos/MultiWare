...
public class DisableCommand extends SubCommand {
    ...
    public DisableCommand(MultiWare plugin) {
        super(plugin);
        this.plugin = plugin;
    }
    ...
    public String getDescription() {
        return "Schakelt een minigame uit";
    }

    @Override
    public String getSyntax() {
        return "/mw disable <minigame>";
    }

    @Override
    public String getPermission() {
        return "multiware.command.disable";
    }
    ...
    public boolean execute(CommandSender sender, String[] args) {
        ...
        if (!config.isEnabled()) {
            sender.sendMessage(ChatColor.YELLOW + "Minigame '" + minigameName + "' is al uitgeschakeld!");
            return true;
        }

        config.setEnabled(false);
        if (plugin.getGameManager().saveMinigameConfig(config)) {
            sender.sendMessage(ChatColor.GREEN + "Minigame '" + minigameName + "' is uitgeschakeld!");
        } else {
            sender.sendMessage(ChatColor.RED + "Er is een fout opgetreden bij het uitschakelen van minigame '" + minigameName + "'!");
        }
